...
def signal_types(
    ...
    results = ExplicitGapSignalExtractor().extract(evidence)

    return {result.signal_type for result in results}
...
def test_extracts_explicit_limitation():
    ...
    assert GapSignalType.LIMITATION in signals
...
def test_extracts_future_work():
    ...
    assert GapSignalType.FUTURE_WORK in signals


def test_extracts_unresolved_problem():

    signals = signal_types(
        ("Generating high-quality tests for " "complex branches remains challenging."),
        section="introduction",
    )

    assert GapSignalType.UNRESOLVED_PROBLEM in signals


def test_generic_failure_is_not_gap_signal():

    signals = signal_types(("The generated test failed " "during execution."))
...
def test_generic_limited_word_is_not_gap_signal():
    ...
    signals = signal_types(("The method achieved limited " "branch coverage."))
...
def test_preserves_provenance():
    ...
    evidence = make_evidence(
        ("Future work will investigate " "additional datasets."),
        section="conclusion",
        evidence_id=("08_telpa_chunk_0042"),
    )
    ...
    assert result.evidence_id == "08_telpa_chunk_0042"

    assert result.paper_id == "paper_01"
...
def test_section_bonus_increases_score():
    ...
    extractor = ExplicitGapSignalExtractor()

    text = "Future work will investigate " "additional datasets."
    ...
    assert preferred[0].relevance_score > nonpreferred[0].relevance_score
...
def test_future_work_accepts_planned_research_action():
    ...
    signals = extractor.extract(evidence)

    assert any(signal.signal_type == GapSignalType.FUTURE_WORK for signal in signals)
...
def test_future_work_accepts_exploration_direction():
    ...
    evidence = make_evidence(
        text=(
            "Future work can explore techniques " "to improve generated test oracles."
        ),
        section="conclusion",
    )
...
def test_future_work_rejects_document_navigation():
    ...
    assert not any(
        signal.signal_type == GapSignalType.FUTURE_WORK for signal in signals
    )
