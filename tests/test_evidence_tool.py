...
class FakeEvidenceStore:
    ...
    def load_chunks(
        ...
        text_a = "Mutation testing evaluates " "the effectiveness of tests."

        text_b = "Coverage feedback guides " "iterative test generation."
...
def test_get_evidence_resolves_chunk():
    ...
    tool = EvidenceTool(store=FakeEvidenceStore())

    evidence = tool.get("paper_a_chunk_0001")

    assert evidence.evidence_id == "paper_a_chunk_0001"

    assert evidence.paper_id == "paper_a"
...
def test_get_rejects_empty_id():
    ...
    with pytest.raises(
        ValueError,
        match=("evidence_id cannot be empty"),
    ):
