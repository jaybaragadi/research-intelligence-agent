...
TEST_QUERY = (
    "How do LLM-based approaches improve " "automated software test generation?"
)
...
def test_real_pipeline_generates_review(
    ...
    assert real_review.paper_ids == TEST_PAPERS
...
def test_real_pipeline_grounding_is_valid(
    ...
    assert real_review.validation is not None

    assert real_review.validation.valid is True

    assert real_review.validation.issues == []
...
def test_real_pipeline_generates_all_review_sections(
    ...
    assert len(real_review.sections) == 8
...
def test_real_pipeline_contains_supported_findings(
    ...
    findings = [
        finding for section in real_review.sections for finding in section.findings
    ]

    assert findings

    assert any(finding.evidence_ids for finding in findings)
...
def test_real_pipeline_evidence_has_provenance(
    ...
    evidence_items = [
        evidence for section in real_review.sections for evidence in section.evidence
    ]
    ...
    for evidence in evidence_items:
        ...
        assert evidence.paper_id in TEST_PAPERS

        assert evidence.page_number > 0

        assert evidence.text.strip()

        assert evidence.citation_text.strip()
...
def test_real_pipeline_citations_are_present(
    ...
    assert all(citation.strip() for citation in real_review.citations)
...
def test_real_pipeline_renders_markdown(
    ...
    markdown = LiteratureReviewMarkdownRenderer().render(real_review)

    assert "# Integration Test Review" in markdown

    assert "## Scope Note" in markdown

    assert "## Grounding Validation" in markdown
...
def test_real_pipeline_markdown_contains_evidence_markers(
    ...
    assert any(f"[{evidence_id}]" in markdown for evidence_id in evidence_ids)
...
def test_real_pipeline_scope_note_is_preserved(
    ...
    assert "indexed corpus only" in real_review.corpus_scope_note.lower()
