...
def test_end_to_end_metrics_valid_result():
    ...
    assert metrics.mean_stage_success_rate == 1.0

    assert metrics.answer_success_rate == 1.0

    assert metrics.comparison_success_rate == 1.0

    assert metrics.gap_analysis_success_rate == 1.0

    assert metrics.literature_review_success_rate == 1.0

    assert metrics.mean_comparison_paper_coverage == 1.0

    assert metrics.mean_gap_paper_coverage == 1.0

    assert metrics.mean_literature_review_paper_coverage == 1.0
...
def test_end_to_end_metrics_totals():
    ...
    assert metrics.total_answer_claims == 5

    assert metrics.total_answer_evidence == 8

    assert metrics.total_comparison_findings == 4

    assert metrics.total_gap_candidates == 2

    assert metrics.total_review_findings == 5

    assert metrics.total_review_citations == 20

    assert metrics.total_shared_evidence_ids == 4
