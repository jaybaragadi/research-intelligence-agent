    ...
    def analyze(
        ...
        candidates = [
            GapCandidate(
                ...
                description=("A reported limitation " "was found."),
...
def test_gap_evaluator_valid_case():
    ...
    evaluator = GapEvaluator(gap_service=FakeGapService())

    result = evaluator.evaluate_case(case)

    assert result.signal_paper_coverage == 1.0

    assert result.signal_count == 1

    assert result.candidate_count == 1

    assert result.explicit_candidate_count == 1

    assert result.corpus_imbalance_candidate_count == 0

    assert result.insufficient_evidence_candidate_count == 0

    assert result.candidate_evidence_reference_integrity == 1.0

    assert result.missing_candidate_evidence_references == []

    assert result.invalid_candidate_paper_references == []

    assert result.dimension_population_rate == 0.5
...
def test_gap_metrics():
    ...
    result = GapEvaluator(gap_service=FakeGapService()).evaluate_case(case)
    ...
    assert metrics.mean_signal_paper_coverage == 1.0

    assert metrics.backend_validation_pass_rate == 1.0

    assert metrics.candidate_evidence_reference_integrity == 1.0

    assert metrics.mean_dimension_population_rate == 0.5

    assert metrics.total_candidates == 1

    assert metrics.total_explicit_candidates == 1
