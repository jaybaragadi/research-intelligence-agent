...
def test_extract_metric_signal():
    ...
    paper = create_paper(
        "The generated test suite achieved " "82 percent branch coverage."
    )

    signals = extract_research_signals(paper)
...
def test_extract_limitation_signal():
    ...
    paper = create_paper(
        "One limitation of our approach " "is its dependence on source-code context."
    )
