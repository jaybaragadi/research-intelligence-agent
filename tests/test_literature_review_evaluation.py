    ...
    def generate(
        ...
        for index, section_type in enumerate(
            ...
            finding = LiteratureReviewFinding(
                ...
                statement=(
                    "Evidence-backed review finding."
                ),
                paper_ids=[
                    "03_mutap"
                ],
...
def test_literature_review_evaluator_valid_case():
    ...
    assert (
        result.section_structure_complete
        is True
    )
    ...
    assert (
        result.finding_evidence_coverage
        == 1.0
    )
    ...
    assert (
        result.corpus_scope_note_present
        is True
    )
...
def test_literature_review_metrics():
    ...
    assert (
        metrics.mean_review_paper_coverage
        == 1.0
    )

    assert (
        metrics.section_structure_pass_rate
        == 1.0
    )

    assert (
        metrics.mean_finding_evidence_coverage
        == 1.0
    )
    ...
    assert (
        metrics.corpus_scope_note_pass_rate
        == 1.0
    )