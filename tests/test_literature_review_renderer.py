...
def build_review():
    ...
    section = LiteratureReviewSection(
        ...
        objective=("Summarize generation strategies."),
        findings=[],
        evidence=[evidence],
        narrative=("The indexed study uses " "structured test generation. [E1]"),
    )

    return LiteratureReview(
        query=("How are LLMs used for " "test generation?"),
        paper_ids=["03_mutap"],
        title=("LLMs for Software Testing"),
        sections=[section],
        citations=["MuTAP (2023), p. 5"],
        validation=(
            LiteratureReviewValidationResult(
                valid=True,
                issues=[],
...
def test_renderer_contains_title():
    ...
    markdown = LiteratureReviewMarkdownRenderer().render(build_review())

    assert "# LLMs for Software Testing" in markdown
...
def test_renderer_contains_query():
    ...
    assert "How are LLMs used for " "test generation?" in markdown
...
def test_renderer_contains_section():
    ...
    assert "## Test Generation Strategies" in markdown
...
def test_renderer_contains_page_and_section():
    ...
    assert "p. 5, methodology" in markdown
...
def test_renderer_contains_citation_index():
    ...
    assert "MuTAP (2023), p. 5" in markdown
...
def test_renderer_contains_scope_note():
    ...
    assert "indexed corpus only" in markdown
...
def test_renderer_collapses_evidence_whitespace():
    ...
    markdown = LiteratureReviewMarkdownRenderer().render(review)

    assert "Mutation feedback guides " "test generation." in markdown
