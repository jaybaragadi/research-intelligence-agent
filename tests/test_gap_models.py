...
def test_gap_type_values_are_stable():
    ...
    assert GapType.EXPLICIT.value == "explicit"

    assert GapType.CORPUS_IMBALANCE.value == "corpus_imbalance"

    assert GapType.INSUFFICIENT_EVIDENCE.value == "insufficient_evidence"
...
def test_gap_evidence_preserves_provenance():
    ...
    evidence = GapEvidence(
        ...
        text=("A limitation of the " "approach is..."),
    ...
    assert evidence.evidence_id == "03_mutap_chunk_0001"
...
def test_gap_candidate_preserves_support():
    ...
    candidate = GapCandidate(
        ...
        title=("Limited evaluation scope"),
        description=("The paper reports a " "restricted evaluation."),
        confidence=(GapConfidence.HIGH),
        paper_ids=["03_mutap"],
        evidence_ids=["03_mutap_chunk_0001"],
        dimensions=["evaluation_method"],
        reason=("Explicit limitation " "language was found."),
    )

    assert candidate.gap_id == "G1"

    assert candidate.evidence_ids == ["03_mutap_chunk_0001"]
