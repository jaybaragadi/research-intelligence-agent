...
def test_analyzes_dimension_coverage():
    ...
    results = CorpusCoverageAnalyzer().analyze(evidence)

    lookup = {result.dimension: result for result in results}

    quality = lookup["quality_objective"]

    assert quality.paper_count == 2
...
def test_requested_missing_dimension_is_preserved():
    ...
    results = CorpusCoverageAnalyzer().analyze_for_dimensions(
        evidence=evidence,
        dimensions=[
            "quality_objective",
            "limitations",
    ...
    missing = lookup["limitations"]

    assert missing.paper_count == 0

    assert missing.evidence_count == 0

    assert missing.paper_ids == []
