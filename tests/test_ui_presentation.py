...
def test_search_score_label():
    ...
    assert search_score_label(0.812345) == "0.8123"


def test_result_section_label():

    assert result_section_label("methodology") == "methodology"


def test_result_section_label_missing():

    assert result_section_label(None) == "Unspecified"


def test_review_section_label():

    assert review_section_label("feedback_and_iteration") == "Feedback And Iteration"


def test_literature_review_filename():

    assert (
        literature_review_filename("MuTAP and CoverUp Literature Review")
...
def test_literature_review_filename_empty():
    ...
    assert literature_review_filename("") == "literature_review.md"
...
def test_review_validation_status_valid():
    ...
    assert review_validation_status(Validation()) == "Valid"
...
def test_review_validation_status_invalid():
    ...
    assert review_validation_status(Validation()) == "Invalid"


def test_review_validation_status_none():

    assert review_validation_status(None) == "Not Available"


def test_gap_type_label_explicit():

    assert gap_type_label("explicit") == "Explicit Gap"


def test_gap_type_label_corpus_imbalance():

    assert gap_type_label("corpus_imbalance") == "Corpus Imbalance"


def test_gap_type_label_insufficient_evidence():

    assert gap_type_label("insufficient_evidence") == "Insufficient Evidence"


def test_gap_confidence_label():

    assert gap_confidence_label("high") == "High"


def test_signal_type_label():

    assert signal_type_label("future_work") == "Future Work"
...
def test_evidence_title():
    ...
    assert result == ("03_mutap_chunk_0085 | " "03_mutap | Page 13")


def test_evidence_section():

    assert evidence_section("methodology") == "methodology"


def test_missing_evidence_section():

    assert evidence_section(None) == "Unspecified"


def test_grounding_status_valid():

    assert grounding_status(True) == "Yes"


def test_grounding_status_invalid():

    assert grounding_status(False) == "No"


def test_dimension_label_known_dimension():

    assert dimension_label("generation_strategy") == "Generation Strategy"


def test_dimension_label_unknown_dimension():

    assert dimension_label("new_research_dimension") == "New Research Dimension"


def test_comparison_cell_status_with_summary():

    assert (
        comparison_cell_status("Uses mutation feedback.") == "Uses mutation feedback."
    )


def test_comparison_cell_status_without_summary():

    result = comparison_cell_status(None)

    assert "No matching validated evidence" in result
