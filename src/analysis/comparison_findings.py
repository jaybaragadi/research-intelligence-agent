...
class ComparisonFindingBuilder:
    ...
    ) -> list[ComparativeFinding]:

        findings: list[ComparativeFinding] = []

        for row in rows:

            populated = [cell for cell in row.cells if cell.summary]

            if len(populated) < 2:
                continue

            evidence_ids = [
                evidence_id for cell in populated for evidence_id in cell.evidence_ids
            ]

            paper_ids = [cell.paper_id for cell in populated]
