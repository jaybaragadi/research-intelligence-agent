...
EXPLICIT_GAP_QUERIES = (
    ("limitations threats to validity " "weaknesses constraints drawbacks"),
    ("future work future research " "future directions further investigation"),
    (
...
class ExplicitGapEvidenceRetriever:
    ...
    def retrieve(
        ...
        if evidence_per_query <= 0:
            ...
            raise ValueError("evidence_per_query must be positive")
        ...
        for paper_id in cleaned_papers:
            ...
            for query in EXPLICIT_GAP_QUERIES:

                response = self.search_tool.search(
                    query=query,
                    top_k=(evidence_per_query),
                    allowed_paper_ids={paper_id},
                    max_per_paper=(evidence_per_query),
                )
