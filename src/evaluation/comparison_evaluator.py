...
class ComparisonEvaluator:
    ...
    def evaluate_case(
        ...
        profile_papers = self._unique_in_order(
            [profile.paper_id for profile in analysis.profiles]
        )

        matrix_papers = self._matrix_papers(analysis)

        missing_profile_papers = [
            paper_id for paper_id in case.paper_ids if paper_id not in profile_papers
        ]

        missing_matrix_papers = [
            paper_id for paper_id in case.paper_ids if paper_id not in matrix_papers
        ]

        profile_paper_coverage = (
            len(requested_set & set(profile_papers)) / len(requested_set)
            if requested_set
            else 0.0
        )

        matrix_paper_coverage = (
            len(requested_set & set(matrix_papers)) / len(requested_set)
            if requested_set
            else 0.0
        )

        profile_evidence_ids = self._profile_evidence_ids(analysis)

        profile_evidence_set = set(profile_evidence_ids)
        ...
        for row in analysis.matrix:
            ...
            for cell in row.cells:
                ...
                if cell.summary or cell.evidence_ids:
                ...
                for evidence_id in cell.evidence_ids:
                    ...
                    if evidence_id not in profile_evidence_set:

                        missing_cell_evidence_references.add(evidence_id)

        empty_cell_count = matrix_cell_count - populated_cell_count

        matrix_population_rate = (
            populated_cell_count / matrix_cell_count if matrix_cell_count else 0.0
        )
        ...
        cell_evidence_reference_integrity = self._reference_integrity(
            total_references=(cell_evidence_reference_count),
            missing_reference_count=len(missing_cell_evidence_references),
        )
        ...
        return ComparisonEvaluationResult(
            ...
            missing_profile_papers=(missing_profile_papers),
            missing_matrix_papers=(missing_matrix_papers),
            profile_paper_coverage=(profile_paper_coverage),
            matrix_paper_coverage=(matrix_paper_coverage),
            dimension_count=len(analysis.matrix),
            matrix_cell_count=(matrix_cell_count),
            populated_cell_count=(populated_cell_count),
            empty_cell_count=(empty_cell_count),
            matrix_population_rate=(matrix_population_rate),
            profile_evidence_count=len(profile_evidence_ids),
            profile_evidence_ids=(profile_evidence_ids),
            populated_cell_evidence_reference_count=(cell_evidence_reference_count),
            missing_cell_evidence_references=sorted(missing_cell_evidence_references),
            cell_evidence_reference_integrity=(cell_evidence_reference_integrity),
    ...
    def _profile_evidence_ids(
        ...
        return self._unique_in_order(evidence_ids)
    ...
    def _matrix_papers(
        ...
        return self._unique_in_order(paper_ids)
