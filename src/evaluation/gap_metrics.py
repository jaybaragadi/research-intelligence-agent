...
def calculate_gap_metrics(
    results: list[GapEvaluationResult],
) -> GapAggregateMetrics:
    ...
    total_candidate_references = sum(
        result.candidate_evidence_reference_count for result in results
    )

    total_missing_candidate_references = sum(
        len(result.missing_candidate_evidence_references) for result in results
    )

    if total_candidate_references:

        evidence_integrity = (
            total_candidate_references - total_missing_candidate_references
    ...
    return GapAggregateMetrics(
        ...
        mean_signal_paper_coverage=sum(
            result.signal_paper_coverage for result in results
        )
        / count,
        ...
        / count,
        candidate_evidence_reference_integrity=(evidence_integrity),
        mean_dimension_population_rate=sum(
            result.dimension_population_rate for result in results
        )
        / count,
        total_candidates=sum(result.candidate_count for result in results),
        total_explicit_candidates=sum(
            result.explicit_candidate_count for result in results
        ),
        total_corpus_imbalance_candidates=sum(
            result.corpus_imbalance_candidate_count for result in results
        ),
        total_insufficient_evidence_candidates=sum(
            result.insufficient_evidence_candidate_count for result in results
        ),
    )
