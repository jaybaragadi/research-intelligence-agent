...
class GroundingEvaluator:
    ...
    def evaluate_case(
        ...
        evidence_by_id = {
            evidence.evidence_id: evidence for evidence in answer.evidence
        }
        ...
        for claim in answer.claims:
            ...
            for evidence_id in claim.evidence_ids:
                ...
                if evidence_id not in evidence_by_id:
                    ...
                    missing_evidence_references.add(evidence_id)
        ...
        for evidence in answer.evidence:
            ...
            if self._has_complete_provenance(evidence):
        ...
        evidence_papers = self._unique_in_order(
            [evidence.paper_id for evidence in answer.evidence]
        )

        expected = set(case.expected_papers)

        matched_expected_papers = [
            paper_id for paper_id in evidence_papers if paper_id in expected
        ]
        ...
        claim_evidence_coverage = (
            claims_with_evidence / claim_count if claim_count else 0.0
        )

        provenance_completeness = (
            complete_provenance / evidence_count if evidence_count else 0.0
        )

        expected_paper_recall = (
            len(set(matched_expected_papers)) / len(expected) if expected else 0.0
        )
        ...
        return GroundingEvaluationResult(
            ...
            missing_evidence_references=sorted(missing_evidence_references),
            evidence_with_complete_provenance=(complete_provenance),
            evidence_with_incomplete_provenance=(incomplete_provenance),
            backend_validation_valid=(answer.validation.is_valid),
            backend_validation_issue_count=(answer.validation.issue_count),
            evidence_papers=evidence_papers,
            expected_papers=(case.expected_papers),
            matched_expected_papers=(matched_expected_papers),
            claim_evidence_coverage=(claim_evidence_coverage),
            provenance_completeness=(provenance_completeness),
            expected_paper_recall=(expected_paper_recall),
        )
