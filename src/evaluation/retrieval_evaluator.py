...
class RetrievalEvaluator:
    ...
    def evaluate_case(
        ...
        retrieved_papers = [result.paper_id for result in response.results]

        unique_retrieved_papers = self._unique_in_order(retrieved_papers)

        expected = set(case.expected_papers)

        matched = [
            paper_id for paper_id in unique_retrieved_papers if paper_id in expected
        ]

        missing = [
            paper_id for paper_id in case.expected_papers if paper_id not in matched
        ]
        ...
        return RetrievalEvaluationResult(
            ...
            unique_retrieved_papers=(unique_retrieved_papers),
    ...
    def _hit_at_k(
        ...
        return any(paper_id in expected_papers for paper_id in retrieved_papers[:k])
    ...
    def _recall_at_k(
        ...
        matched = retrieved_at_k & expected_papers
