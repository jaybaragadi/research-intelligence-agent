...
def fallback_title(
    ...
    abstract_position = first_page_text.lower().find("abstract")

    if abstract_position > 0:
        candidate = first_page_text[:abstract_position].strip()
...
def build_profile(
    ...
    if pdf_path.exists():
        ...
        title, authors, year = read_pdf_document_metadata(pdf_path)

    first_page_text = paper.pages[0].text if paper.pages else ""
    ...
    research_questions = extract_research_questions(paper)

    research_signals = extract_research_signals(paper)
...
def run_metadata_extraction() -> MetadataSummary:
    """
    ...
    summary = MetadataSummary(discovered=len(processed_files))
    ...
    for index, processed_path in enumerate(
        ...
        print(f"[{index}/{len(processed_files)}] " f"Profiling {processed_path.name}")

        try:
            profile = build_profile(processed_path)

            output_path = save_paper_profile(
                profile,
            ...
            print(f"    Title      : " f"{profile.title[:80]}")

            print(f"    Authors    : " f"{len(profile.authors)}")

            print(f"    Year       : " f"{profile.year}")

            print(f"    Abstract   : " f"{'Yes' if profile.abstract else 'No'}")

            print(f"    Sections   : " f"{len(profile.sections)}")

            print(f"    RQs        : " f"{len(profile.research_questions)}")

            print(f"    Signals    : " f"{len(profile.research_signals)}")
